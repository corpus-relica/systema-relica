import { getEntityType } from "./entityRetrievalController.js";
...
import { isClassifiedAsP } from "./predicates.js";

const canPlayRoleP = async (entity_uid, role_uid) => {
  const type = await getEntityType(entity_uid);
  let kind_uid;
  if (type === "individual") {
    const classFact = await getClassificationFact(entity_uid);
  ...
  const possibleRolePlayers = await getPossibleRolePlayers(role_uid);
  ...
  while (idx < possibleRolePlayers.length) {
    ...
    const sh = await getSpecializationHierarchy(kind_uid);
...
const validateUIDNamePair = async (uid, name) => {
  ...
  const names = await getNames(parseInt(uid));
  console.log("names:", names);
  return names.includes(name);
};

const validateValidityContext = async (valid_ctx_uid, valid_ctx_name) => {
  ...
  if (
    ...
    const ctxNameAndUIDAlign = await validateUIDNamePair(
      valid_ctx_uid,
      valid_ctx_name,
    );
    ...
    const isAppCtx = await isClassifiedAsP(valid_ctx_uid, APPLICATION_CONTEXT);
    if (!isAppCtx) {
      return {
        isValid: false,
        message:
...
export const simpleValidateBinaryFact = async (fact) => {
  const {
    lh_object_uid,
    lh_object_name,
    rh_object_uid,
  ...
  if (rh_object_uid === "" || rel_type_uid === "" || lh_object_uid === "") {
    return {
      isValid: false,
  ...
  const validityContext = await validateValidityContext(
    valid_ctx_uid,
    valid_ctx_name,
  );
  ...
  const qux = LHUnknownUID
    ? true
    : await validateUIDNamePair(lh_object_uid, lh_object_name);
  const quux = await validateUIDNamePair(rel_type_uid, rel_type_name);
  const bat = await validateUIDNamePair(rh_object_uid, rh_object_name);
  ...
  const role1 = await getRequiredRole1(parseInt(rel_type_uid));
  const role2 = await getRequiredRole2(parseInt(rel_type_uid));
  const foo = await canPlayRoleP(parseInt(lh_object_uid), role1.rh_object_uid);
  message += !foo
    ? `(${lh_object_name}) can't play role1 (${role1.rh_object_name}) of relation (${rel_type_name}).  `
    : "";
  const bar = await canPlayRoleP(parseInt(rh_object_uid), role2.rh_object_uid);
  message += !bar
