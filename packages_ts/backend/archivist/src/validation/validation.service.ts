...
export class ValidationService {
  ...
  async canPlayRoleP(entity_uid, role_uid) {
    const type = await this.entityRetrievalService.getEntityType(entity_uid);
    let kind_uid;
    if (type === 'individual') {
      const classFact =
    ...
    while (idx < possibleRolePlayers.length) {
      ...
      const sh =
        await this.gellishBaseService.getSpecializationHierarchy(kind_uid);
  ...
  async simpleValidateBinaryFact(fact) {
    ...
    const quux = await this.validateUIDNamePair(rel_type_uid, rel_type_name);
    const bat = RHUnknownUID
      ? true
    ...
    // addendum 2024-05-26: DO accept unkown UIDs in the rh_uid position
    // TODO: make sure (elsewhere) that we make sure that the RHUnknownUID is
    // defined earlier in the (presumed) set of facts to be transacted into the knowledge graph
    // if (RHUnknownUID) {
    //   return {
    //     isValid: false,
  ...
  async isClassifiedAsP(indvUID, kindUID) {
    ...
    if (classificationFact) {
      ...
      const specializationHierarchy =
        await this.gellishBaseService.getSpecializationHierarchy(rh_object_uid);
