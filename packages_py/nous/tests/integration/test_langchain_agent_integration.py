...
class TestNOUSAgentInitialization:
    ...
    async def test_nous_agent_with_tools(
        self,
        mock_nous_agent,
...
class TestNOUSAgentConversation:
    ...
    async def test_agent_error_handling(self, mock_nous_agent):
        ...
        with pytest.raises(Exception):
            await mock_nous_agent.handleInput(
        ...
        response = await mock_nous_agent.handleInput(
...
class TestNOUSAgentToolIntegration:
    ...
    async def test_aperture_tool_integration(
        self, mock_nous_agent, mock_aperture_client
    ):
        ...
        async def mock_agent_with_aperture(messages):
            ...
            if "environment" in user_message.lower():
                ...
                env_data = await mock_aperture_client.retrieveEnvironment(
                    "test_env", None
                )
        ...
        env_query = [
            {
                "role": "user",
                "content": "Can you tell me about the current environment?",
            }
        ]
    ...
    async def test_archivist_tool_integration(
        self, mock_nous_agent, mock_archivist_client
    ):
        ...
        memory_query = [
            {
                "role": "user",
                "content": "Do you remember our conversation about AI ethics?",
            }
        ]
    ...
    async def test_clarity_tool_integration(self, mock_nous_agent, mock_clarity_client):
        ...
        # Test analysis query
        analysis_query = [
            {"role": "user", "content": "Can you analyze this text for sentiment?"}
        ]
    ...
    async def test_multi_tool_integration(
        self,
        mock_nous_agent,
        mock_aperture_client,
        mock_archivist_client,
        ...
        multi_query = [
            {
                "role": "user",
                "content": "Can you do a comprehensive analysis of the current situation?",
            }
        ]
...
class TestNOUSWebSocketAgentIntegration:
    ...
    async def test_websocket_agent_error_recovery(self, mock_nous_agent):
        ...
        success_result = await server.handle_user_input(
            success_payload, "success_client"
        )
    ...
    async def test_concurrent_websocket_agent_requests(self, mock_nous_agent):
        ...
        assert (
            len([item for item in processing_order if item.startswith("start_")]) == 3
        )
...
class TestNOUSAgentPerformance:
    ...
    async def test_agent_response_time(self, mock_nous_agent):
        ...
        assert (
            response_time > 0.04
        )  # Should take at least our simulated processing time
    ...
    async def test_agent_memory_usage(self, mock_nous_agent):
        ...
        mock_nous_agent.handleInput.return_value = (
            "I've processed your conversation history."
        )
