...
class NOUSSocketIOServer:
    ...
    def __init__(self):
        ...
        self.sio = socketio.AsyncServer(
            cors_allowed_origins="*", logger=logger, engineio_logger=logger
        )
    ...
    async def handle_process_chat_input(
        self, payload: Dict[str, Any], sid: str
    ) -> Dict[str, Any]:
        ...
        print(f"Extracted from payload - message: '{message}', userId: '{user_id}', context: {context}")
        ...
        try:
            ...
            if hasattr(self, "_nous_user_input_handler"):
                ...
                await self._nous_user_input_handler(
                    user_id, context.get("environmentId", 1), message, sid
                )
            ...
            await self.send_error_message(
                sid, f"Error processing your request: {str(e)}"
            )
