...
export class EntitiesController {
  ...
  async resolveEntitiesGet(@User() user: any, @Query("uids") uids: string) {
    try {
      if (!uids) {
      ...
      if (uidArray.some((uid) => isNaN(uid))) {
        throw new BadRequestException(
          "Invalid UID format. All UIDs must be valid numbers."
        );
      }
  ...
  @ApiResponse({
    status: 200,
    description: "Entity category retrieved successfully",
  })
  ...
  @ApiOperation({
    summary:
      "Retrieve entity collections, the collections defined in the semantic model",
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
    description: "Entity collections retrieved successfully",
  })
