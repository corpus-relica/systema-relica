...
export const getSetupStatus = async (retries = 3): Promise<SetupStatus> => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `Setup status check failed (attempt ${attempt}/${retries}):`,
        error
      );
      ...
      if (
        ...
        try {
          ...
          const response = await PortalAxiosInstance.get(
      ...
      if (attempt === retries) {
        throw new Error(
          `Failed to get setup status after ${retries} attempts: ${error.message}`
        );
      }
...
export const startSetup = async (
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `Start setup failed (attempt ${attempt}/${retries}):`,
        error
      );

      if (attempt === retries) {
        throw new Error(
          `Failed to start setup after ${retries} attempts: ${error.message}`
        );
      }
...
export const createAdminUser = async (
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const response = await PortalAxiosInstance.post(
        "/api/prism/setup/create-admin-user",
        userData
      );
      return response.data;
    } catch (error) {
      console.error(
        `Create admin user failed (attempt ${attempt}/${retries}):`,
        error
      );

      if (attempt === retries) {
        throw new Error(
          `Failed to create admin user after ${retries} attempts: ${error.message}`
        );
      }
...
export const getGuestToken = async (
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(
        `Get guest token failed (attempt ${attempt}/${retries}):`,
        error
      );

      if (attempt === retries) {
        throw new Error(
          `Failed to get guest token after ${retries} attempts: ${error.message}`
        );
      }
...
export const loadUserEnvironment = async (userId: number): Promise<any> => {
  const response = await PortalAxiosInstance.get(
    "/environment/retrieve?userId=" + userId
  );
  return response.data;
};
...
export const resetSystemState = async (): Promise<{
  success: boolean;
  message?: string;
}> => {
  try {
    const response = await PortalAxiosInstance.post(
      "/api/prism/debug/reset-system"
    );
    return response.data;
  } catch (error) {
