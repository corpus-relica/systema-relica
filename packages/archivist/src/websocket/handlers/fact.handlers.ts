...
export class FactHandlers {
  ...
  async handleFactCreate(data: FactCreateMessage, client: Socket) {
    // Service now returns raw data and throws errors
    return await this.factService.submitBinaryFact(data);
  }

  async handleFactUpdate(data: FactUpdateMessage, client: Socket) {
  ...
  async handleFactDelete(data: FactDeleteMessage, client: Socket) {
    await this.factService.deleteFact(data.fact_uid);
    return { fact_uid: data.fact_uid, success: true };
  }

  async handleFactGet(data: FactQueryMessage, client: Socket) {
    return await this.factService.getFactsAboutKind(data.uid);
  }

  async handleFactGetDefinitive(data: FactQueryMessage, client: Socket) {
    return await this.gellishBaseService.getDefinitiveFacts(data.uid);
  }

  async handleGetAllRelated(data: FactQueryMessage, client: Socket) {
    return await this.factService.getAllRelatedFacts(data.uid);
  }

  async handleFactGetSubtypes(data: FactQueryMessage, client: Socket) {
    return await this.factService.getSubtypes(data.uid);
  }

  async handleFactGetSubtypesCone(data: FactQueryMessage, client: Socket) {
    return await this.factService.getSubtypesCone(data.uid);
  }

  async handleFactGetSupertypes(data: FactQueryMessage, client: Socket) {
    // getSupertypesOf method doesn't exist - returning empty result
    return [];
  }

  async handleFactGetClassified(data: FactQueryMessage, client: Socket) {
    return await this.factService.getClassified(data.uid);
  }

  async handleFactValidate(data: FactCreateMessage, client: Socket) {
