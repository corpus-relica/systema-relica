...
export const getLHObjectCompletion = async (rel_type_uid, rh_object_uid) => {
  const lhObjectCompletionQuery = `
MATCH ()-[r]->()
WHERE r.rel_type_uid = $rel_type_uid AND r.rh_object_uid = $rh_object_uid
RETURN r`;
  const result = await execQuery(lhObjectCompletionQuery, {
    rel_type_uid,
    rh_object_uid,
  });
...
export const getRHObjectCompletion = async (lh_object_uid, rel_type_uid) => {
  const rhObjectCompletionQuery = `
MATCH ()-[r]->()
WHERE r.lh_object_uid = $lh_object_uid AND r.rel_type_uid = $rel_type_uid
RETURN r`;
  const result = await execQuery(rhObjectCompletionQuery, {
    lh_object_uid,
    rel_type_uid,
  });
