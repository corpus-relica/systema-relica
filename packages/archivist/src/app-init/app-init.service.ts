...
export class AppInitService implements OnApplicationBootstrap {
  ...
  private async initializeDatabase() {
    ...
    if (isEmpty) {
      ...
      await this.rawFactIngestionService.loadFromFileCreateNodes('1.csv');
      await this.rawFactIngestionService.loadFromFileCreateNodes('2.csv');
  ...
  private async buildSubtypesCache(batchSize = 1000) {
    ...
    while (hasMore) {
      ...
      for (const nodeRecord of nodes) {
        ...
        const uid = this.graphService.resolveNeo4jInt(node.properties.uid);
      ...
      this.logger.verbose(`Processed ${nodeToDescendants.size} descendants`);
    ...
    `);
      ...
      await Promise.all(
        nodes.map(async (nodeRecord) => {
          ...
          const fact_uid = this.graphService.resolveNeo4jInt(raw_fact_uid);
