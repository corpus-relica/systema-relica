...
import { subtypes, classified, factsAboutIndividual } from 'src/graph/queries';
...
export class FactRetrievalService {
    ...
    getFactsAboutKind = async (uid) => {
        ...
        const specH =
            await this.gellishBaseService.getSpecializationHierarchy(uid);
...
`;
        ...
        const results = await this.graphService.execQuery(
            allRelatedFactsQuery,
            {
...
`;
        ...
        const resultsb = await this.graphService.execQuery(
            allRelatedFactsQueryb,
            {
                start_uid: uid1,
                end_uid: uid2,
