...
export class XLSService {
    ...
    resolveTempUIDs(
        ...
        const rh_object_uid_idx = String.fromCharCode(
            64 + header.indexOf('15'),
        );
        ...
        if (ws['!ref']) {
            ...
            console.error(
                'Invalid worksheet format: !ref property is missing.',
            );
        }
            ...
            if (rowIsEmpty) {
                ...
                for (let moveRow = R + 1; moveRow <= xxx.e.r; ++moveRow) {
                    for (let C = xxx.s.c; C <= xxx.e.c; ++C) {
                        ...
                        worksheet[cellToReplaceAddress] =
                            worksheet[cellToMoveAddress];
                    }
                }
