...
const uidFactComponentMap: Record<number, string> = {
	0: 'sequence',
	69: 'language_uid',
	54: 'language',
	71: 'lh_context_uid',
...
export default function LoadUserSpace({next}: Props) {
	...
	useEffect(() => {
		...
		const exec = async () => {
			...
			const ws: XLSX.WorkSheet[] = await readXLSFixDates([
			...
			//@ts-ignore
			const header: number[] = new Array(colCount).fill('x').map((x, idx) => {
				const targ: any = XLSX.utils.encode_cell({c: idx, r: 0});
			...
			for (let i = 1; i < sheetRange.e.r - 1; i++) {
				...
				for (let j = 0; j < colCount; j++) {
					const targ: any = XLSX.utils.encode_cell({c: j, r: i});
					const cell = sheet[targ];
					if (cell) {
						const headerKey: number | undefined = header[j];
						if (!headerKey) continue;
						const factKey: string | undefined = uidFactComponentMap[headerKey];
						if (!factKey) continue;
						...
						switch (factKey) {
							...
							case 'rh_object_name':
								//@ts-ignore
								fact[factKey as keyof Fact] = cell['w'];
								break;
							default:
								//@ts-ignore
								fact[factKey as keyof Fact] = cell['v'];
								break;
						}
						// console.log(
			...
			for (const fact of facts) {
				if (!fact.sequence) fact.sequence = 0;
				if (!fact.partial_definition) fact.partial_definition = '';
				if (!fact.full_definition) fact.full_definition = '';
			}
