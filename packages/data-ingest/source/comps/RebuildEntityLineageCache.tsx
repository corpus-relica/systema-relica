...
import {
	clearEntityLineageCacheComplete,
	addToEntityLineageCache,
} from '../data/redis.js';
	...
	`);
		...
		await Promise.all(
			nodes.records.map(async nodeRecord => {
				...
				const lineage: number[] = await getLineage(lh_object_uid);
				// console.log('Lineage!!!!');
				// console.log(lineage);
...
const RebuildEntityLineageCache = ({next}: Props) => {
	...
	return (
		<Box flexDirection="column" flexGrow={1}>
			<Text>RebuildEntityLineageCache</Text>
