...
export function findSheetBounds(worksheet: XLSX.WorkSheet): {
	s: {c: number; r: number};
	e: {c: number; r: number};
} {
	// Finding maxWidth
...
export const readXLS = async (
	...
	const finalWorksheet = resolveTempUIDs(
		worksheet,
		minFreeEntityUID,
		minFreeFactUID,
	);
...
export const readXLSFixDates = async (
	files: string[],
	log: (x: string) => void = console.log,
) => {
...
export const saveCSVFiles = async (
...
export const readXLSFixDatesAndSaveCSV = async (
	files: string[],
	log: (x: string) => void = console.log,
) => {
	try {
