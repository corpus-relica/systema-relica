...
export const convertToInt = (neo4jInt) => {
    if (typeof neo4jInt === 'number')
        return neo4jInt;
    if (typeof neo4jInt === 'string')
        return parseInt(neo4jInt);
...
export const convertToDate = (neo4jDate) => {
    if (!neo4jDate)
        return undefined;
...
export const loadFromFileCreateNodes = async (fileName, log = console.log) => {
...
export const loadFromFileCreateRelationships = async (fileName) => {
...
export const fetchPathsToRoot = async (uid) => {
    const session = driver.session();
    try {
        const query = `
...
RETURN path`;
const getSpecializationHierarchy = async (uid) => {
    const session = driver.session();
    try {
