...
// setMinFreeEntityUID,
// setMinFreeFactUID,
addToSubtypesCache, } from './data/redis.js';
...
// 	});
// };
const insertOneFact = async (f) => {
    const resolvedFact = f; //await resolveUIDs(f);
            ...
            paths.forEach(path => {
                path.segments.forEach((segment) => {
                    if (segment.start.labels.includes('Fact')) {
                        entitiesToUpdate.add(convertToInt(segment.start.properties.lh_object_uid));
                        entitiesToUpdate.add(convertToInt(segment.start.properties.rh_object_uid));
                    }
                    if (segment.end.labels.includes('Fact')) {
                        entitiesToUpdate.add(convertToInt(segment.end.properties.lh_object_uid));
                        entitiesToUpdate.add(convertToInt(segment.end.properties.rh_object_uid));
                    }
                });
            });
            // console.log('entitiesToUpdate', entitiesToUpdate.values());
            Array.from(entitiesToUpdate).forEach((entityUID) => {
